...
`PI_BG_BASH_TMUX_SOCKET` selects a private socket. Its parent directory must exist. Swarm sets this to `bg.sock` under each worker's temporary directory. Relative socket access avoids macOS socket-path length limits. Without this setting, bg-bash uses tmux's default socket.
