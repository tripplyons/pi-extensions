...
This wrapper loads pinned `@howaboua/pi-codex-conversion@3.0.33` with local dependency patches. It owns local history, notes, and context windows while retaining upstream voice support and Pi's native tools. Upstream voice-only mode does not disable the wrapper's local context engine.
...
patch in a temporary directory without changing the installed runtime.
...
A window change waits for the complete tool batch. Archived windows retain complete messages, including images, and remain searchable after leaving the active prompt. Notes are virtual files, limited to 1 MB per file and 10 MB per snapshot. Standalone stores use one validated snapshot followed by hashed deltas in Pi custom entries, so session storage grows with new content instead of rewriting the full history after every message. They survive restart, compaction, forks, and tree navigation; sibling branches diverge independently. Mixture stores use its checkpoint/blob chain. Runtime state stays outside this repository.
