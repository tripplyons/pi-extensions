	...
	try {
		...
		const provider: Provider = {
			...
			streamSimple: (model, context) => {
				...
				else if (!has(context, "Writer completion report")) content = tool("mixture_control", { action: "delegate", task: has(context, "Second branch task") ? "Complete second branch work" : "Complete first branch work", nextAction: "Complete the selected branch task and report", successCriteria: ["Report completion"] });
