 ...
 * - Adds the jump-climb strategy to every active autoresearch system prompt
 * - Points the agent at .auto/prompt.md on every active turn
 */
...
const AUTORESEARCH_STRATEGY_HEADING =
  "## Jump-climb: make the largest plausible improvement at each scale";

function readAutoresearchStrategyPrompt(): string {
  const skill = fs.readFileSync(AUTORESEARCH_CREATE_SKILL_PATH, "utf-8");
  const start = skill.indexOf(`${AUTORESEARCH_STRATEGY_HEADING}\n`);
  if (start === -1) {
    throw new Error(`${AUTORESEARCH_CREATE_SKILL_PATH} is missing ${AUTORESEARCH_STRATEGY_HEADING}`);
  }

  const end = skill.indexOf("\n## ", start + AUTORESEARCH_STRATEGY_HEADING.length);
  if (end === -1) {
    throw new Error(`${AUTORESEARCH_CREATE_SKILL_PATH} has no section after ${AUTORESEARCH_STRATEGY_HEADING}`);
  }
  return skill.slice(start, end).trim();
}

const AUTORESEARCH_STRATEGY_PROMPT = readAutoresearchStrategyPrompt();
...
export default function autoresearchExtension(pi: ExtensionAPI) {
  ...
  const composeResumeMessage = (_ctx: ExtensionContext): string => {
    return [
      ...
      "Use the persisted autoresearch state as needed, test the highest-impact viable idea, then call run_experiment + log_experiment.",
      BENCHMARK_GUARDRAIL,
    ].join(" ");
  };
  ...
  const autoresearchHelp = () =>
    [
      "Usage: /autoresearch <start <goal>|resume [context]|pause|clear|dashboard>",
      "",
      "start <goal> enters autoresearch mode and starts or resumes the loop.",
      "resume [context] re-enters autoresearch mode from .auto/prompt.md.",
      "pause leaves autoresearch mode but keeps .auto/log.jsonl intact.",
      "clear deletes the session log (.auto/log.jsonl), resets state, and leaves autoresearch mode.",
      "dashboard opens a local live dashboard for the session log in your browser.",
      "",
      "Examples:",
      "  /autoresearch start optimize unit test runtime, monitor correctness",
      "  /autoresearch resume",
  ...
  // Keep the search strategy in every active turn, including setup and
  // post-compaction turns. The source section also ships in the setup skill.
  pi.on("before_agent_start", async (event, ctx) => {
    ...
    let extra =
      ...
      "\nStarting and finalizing are separate jobs. Leave the research branch and .auto state intact for the user." +
      "\nNever finalize, split, merge, return to trunk, or clean up unless the user explicitly asks." +
      `\n\n${AUTORESEARCH_STRATEGY_PROMPT}` +
  ...
  function packageRoot(): string {
    ...
    // Assets and skills live beside this extension's entry point.
    cachedPackageRoot = extensionDir;
  ...
  pi.registerCommand("autoresearch", {
    description: "Start, pause, clear, or dashboard an autoresearch loop (start <goal> | resume | pause | clear | dashboard)",
    handler: async (args, ctx) => {
      const runtime = getRuntime(ctx);
      const trimmedArgs = (args ?? "").trim();
      const spaceIdx = trimmedArgs.search(/\s/);
      const verb = (spaceIdx === -1 ? trimmedArgs : trimmedArgs.slice(0, spaceIdx)).toLowerCase();
      const rest = (spaceIdx === -1 ? "" : trimmedArgs.slice(spaceIdx + 1)).trim();
      ...
      if (verb === "pause") {
        ...
        ctx.ui.notify(
          wasRunning ? "Autoresearch mode PAUSED — aborting current run" : "Autoresearch mode PAUSED",
          "info"
        );
        return;
      }
      ...
      if (verb === "clear") {
        ...
        if (deletedPaths.length > 0) {
          ctx.ui.notify(`Deleted ${deletedPaths.join(", ")} and left autoresearch mode`, "info");
        } else {
      ...
      if (verb === "start" || verb === "resume") {
        if (verb === "start" && !rest) {
          ctx.ui.notify("Usage: /autoresearch start <goal> — describe what to optimize.", "error");
          return;
        }
        const goal = rest;

        if (runtime.autoresearchMode) {
          ctx.ui.notify("Autoresearch already active — use '/autoresearch pause' first", "info");
          return;
        }
        ...
        const kickoff = rulesLoaded
          ? `Autoresearch mode active. ${goal} ${BENCHMARK_GUARDRAIL}`
          : `/skill:autoresearch-create ${goal} ${BENCHMARK_GUARDRAIL}`.replace(/\s+/g, " ").trim();
      ...
      // Bare text no longer starts the loop — require an explicit verb.
      ctx.ui.notify(`Use /autoresearch start <goal> to begin — bare text no longer starts the loop.\n\n${autoresearchHelp()}`, "info");
    },
  });
}
