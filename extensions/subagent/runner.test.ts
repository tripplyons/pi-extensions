...
test("real Pi child activates Code before prompting and its process can be cancelled", async () => {
  const dir = await mkdtemp(join(tmpdir(), "pi-code-child-"));
  ...
  try {
    ...
    await writeFile(join(agentDir, "pi-codex-conversion.json"), JSON.stringify({ executionMode: "code", voiceFeaturesOnly: false }));
