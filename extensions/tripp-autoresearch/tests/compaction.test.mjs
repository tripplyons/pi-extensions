...
test("summary contains all persisted sources when present", () => {
  withTempWorkDir((workDir) => {
    ...
    assert.match(summary, /Rank the ideas backlog and latest `next:` hints by plausible relative improvement\./);
    assert.match(summary, /Test the highest-impact viable idea next\./);
...
test("summary degrades gracefully when no files exist", () => {
  withTempWorkDir((workDir) => {
    ...
    assert.match(summary, /No runs yet\. Measure the untouched baseline first\./);
    assert.match(summary, /## Next Step/);
  });
});
