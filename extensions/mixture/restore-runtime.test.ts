...
test("real Pi compaction and disk reload preserve role histories and current files without replay", async () => {
	...
	try {
		...
		const prior = materializeCheckpoint(session.sessionManager.getBranch()).checkpoint!.state;
		expect(prior.writer.messages.some(message => message.role === "toolResult" && message.toolCallId === "edit")).toBe(true);
		...
		const restored = materializeCheckpoint(session.sessionManager.getBranch()).checkpoint!.state;
		expect(restored.rootCompactionId).toBe(rootCompaction.id);
		expect(restored.writer.calls).toBe(4);
		expect(restored.writer.messages.filter(message => message.role === "toolResult" && message.toolCallId === "edit")).toHaveLength(1);
