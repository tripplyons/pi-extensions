...
describe("zsh execution and persistent tmux", () => {
	...
	tmuxTest("requires all scope to write to or kill a foreign running job", async () => {
		...
		try {
			...
			if (job) {
				Bun.spawnSync(["tmux", "kill-session", "-t", job.tmuxSession]);
	...
	tmuxTest("survives extension shutdown and is recovered by a new instance", async () => {
		...
		Bun.spawnSync(["tmux", "set-option", "-t", job.tmuxSession, "remain-on-exit", "on"]);
		...
		try {
			...
			expect(Bun.spawnSync(["tmux", "has-session", "-t", job.tmuxSession]).exitCode).not.toBe(0);
		} finally {
			Bun.spawnSync(["tmux", "kill-session", "-t", job.tmuxSession]);
			const cacheHome = process.env.XDG_CACHE_HOME ?? join(homedir(), ".cache");
	...
	tmuxTest("closes its tmux session when the command exits", async () => {
		...
		try {
			for (let attempt = 0; attempt < 100; attempt++) {
				if (Bun.spawnSync(["tmux", "has-session", "-t", job.tmuxSession]).exitCode !== 0) break;
				await Bun.sleep(25);
			}
