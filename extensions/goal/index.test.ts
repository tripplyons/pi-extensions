...
const createHarness = () => {
	...
	return {
		...
		sentMessages,
		notifications,
		emit: (event: string, data: unknown) => pi.events.emit(event, data),
...
describe("goal lifecycle", () => {
	...
	test("holds a goal continuation until Codex compaction commits and the agent settles", async () => {
		...
		harness.emit("tripp:codex-compaction:v1:started", transaction);
		await harness.handlers.get("agent_start")?.({}, harness.ctx);
		await harness.handlers.get("agent_end")?.({ messages: [{ role: "assistant", stopReason: "aborted" }] }, harness.ctx);
		await harness.handlers.get("agent_settled")?.({}, harness.ctx);
		...
		harness.emit("tripp:codex-compaction:v1:committed", {
			...transaction,
	...
	test("pauses an active goal when Codex compaction fails", async () => {
		...
		harness.emit("tripp:codex-compaction:v1:started", transaction);
		harness.emit("tripp:codex-compaction:v1:failed", {
			...transaction,
	...
	test("uses status cards for every goal tool", () => {
		const harness = createHarness();
	...
	test("throws failed goal operations so status cards render errors", async () => {
		const harness = createHarness();
		await harness.handlers.get("session_start")?.({}, harness.ctx);
