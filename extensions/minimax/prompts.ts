...
Use read (1-based lines), edit, write, grep, glob, and bash for local work. Foreground Bash defaults to 120 seconds, capped at 300. After 15 seconds it can return a task ID for the same process; its original deadline still applies. run_in_background starts a managed task immediately, with a default 30-minute deadline. A returned task ID means the command is already running: do not rerun it. Use task_query, task_output, or task_stop. Unobserved completions notify and resume the owning conversation in a batch; do not poll frequently. A task tool returning terminal status acknowledges that completion and suppresses its pending notification. task_output uses byte offsets, not page numbers; omit offset consistently for an automatic cursor, or pass next_offset explicitly. Reading or waiting does not stop the task.
