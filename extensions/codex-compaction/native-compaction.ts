...
import { CODEX_NATIVE_COMPACTION_KIND } from "./protocol.ts";
import { projectContext, type CompressionMode } from "../context-compression/core.ts";

export const NATIVE_COMPACTION_KIND = CODEX_NATIVE_COMPACTION_KIND;
...
export interface NativeCompactionDetails {
	...
	version: typeof NATIVE_COMPACTION_VERSION | typeof LEGACY_NATIVE_COMPACTION_VERSION;
	modelKey: string;
...
class ToolPairingError extends Error {}
...
export function parseNativeCompactionDetails(value: unknown): NativeCompactionDetails | undefined {
	...
	if (
		...
		|| (value.version !== NATIVE_COMPACTION_VERSION && value.version !== LEGACY_NATIVE_COMPACTION_VERSION)
	) return undefined;
	...
	return {
		...
		...(value.version === NATIVE_COMPACTION_VERSION
			? {
				checkpointId: value.checkpointId as string,
				sessionId: value.sessionId as string,
				sourceLeafId: value.sourceLeafId as string,
			}
			: {}),
	};
}
...
function messagesToResponseItems(model: Model<any>, messages: Message[], tools: ToolInfo[]): ResponseItem[] {
	...
	const pendingToolCalls = new Map<string, { callId: string; itemIndex: number }>();
	const ignoredToolCalls = new Set<string>();
	const dropUnresolvedToolCalls = () => {
		if (pendingToolCalls.size === 0) return;
		const itemIndexes = [...pendingToolCalls.values()]
			.map(({ itemIndex }) => itemIndex)
			.sort((left, right) => right - left);
		for (const itemIndex of itemIndexes) items.splice(itemIndex, 1);
		...
		if (message.role === "user") {
			...
			if (message.stopReason === "error" || message.stopReason === "aborted") {
				for (const block of message.content) {
					if (isJsonObject(block) && block.type === "toolCall" && typeof block.id === "string") {
						ignoredToolCalls.add(block.id);
					}
				}
				messageIndex++;
				continue;
			...
			for (const block of message.content) {
				...
				if (block.type === "toolCall" && typeof block.id === "string") {
					...
					pendingToolCalls.set(block.id, { callId, itemIndex: items.length - 1 });
				}
			}
		} else if (message.role === "toolResult" && typeof message.toolCallId === "string") {
			if (ignoredToolCalls.delete(message.toolCallId)) {
				messageIndex++;
				continue;
			}
			const pending = pendingToolCalls.get(message.toolCallId);
			if (!pending) {
				throw new ToolPairingError(`Cannot compact orphaned tool output: ${message.toolCallId}.`);
			}
			pendingToolCalls.delete(message.toolCallId);
			items.push({ type: "function_call_output", call_id: pending.callId, output: toolResultOutput(message, model) });
...
function entriesToResponseItems(
	model: Model<any>, entries: SessionEntry[], tools: ToolInfo[], branch: SessionEntry[], compression?: CompressionMode,
): ResponseItem[] {
	const messages = entries.flatMap((entry) => sessionEntryToContextMessages(entry));
	const projected = compression ? projectContext(messages, branch, compression).messages : messages;
	return messagesToResponseItems(model, convertToLlm(projected), tools);
}

function rebuildLegacyInput(model: Model<any>, branch: SessionEntry[], tools: ToolInfo[], compression?: CompressionMode): ResponseItem[] {
	const rawEntries = branch.filter((entry) =>
		entry.type !== "compaction"
		&& !(entry.type === "custom" && entry.customType === NATIVE_COMPACTION_KIND)
	);
	return entriesToResponseItems(model, rawEntries, tools, branch, compression);
}
...
export function effectiveInputForBranch(params: {
	...
	if (checkpoint.status === "valid") {
		...
		if (
			checkpoint.checkpoint.details.version === NATIVE_COMPACTION_VERSION
			&& params.sessionId
			&& checkpoint.checkpoint.details.sessionId !== params.sessionId
		) {
			throw new Error("The latest OpenAI Codex native compaction checkpoint belongs to a different session.");
		}
		const tail = branch.slice(checkpoint.checkpoint.entryIndex + 1);
		try {
			return [
				...checkpoint.checkpoint.details.replacementHistory.map(cloneItem),
				...entriesToResponseItems(params.model, tail, params.tools, branch, params.compression),
			];
		} catch (error) {
			if (
				checkpoint.checkpoint.details.version !== LEGACY_NATIVE_COMPACTION_VERSION
				|| !(error instanceof ToolPairingError)
			) throw error;
			return rebuildLegacyInput(params.model, branch, params.tools, params.compression);
		}
	}

	const context = buildSessionContext(branch);
	const messages = params.compression ? projectContext(context.messages, branch, params.compression).messages : context.messages;
...
export function buildCompactionRequestBody(params: {
	...
	if (params.model.id === "gpt-6-astra") {
		const previousReasoning = isJsonObject(base.reasoning) ? base.reasoning : undefined;
		body.reasoning = {
			effort: "low",
			summary: typeof previousReasoning?.summary === "string" ? previousReasoning.summary : "auto",
		};
...
export function buildCodexHeaders(params: {
	...
	headers.set(
		...
		params.serviceTier ? `model=${params.modelId};tier=${params.serviceTier}` : `model=${params.modelId}`,
	);
...
async function readStream(
	...
	try {
		return await abortable(reader.read(), signal);
	} catch (error) {
...
function encodeRequestBody(body: JsonObject, headers: Headers): string | Uint8Array {
	const json = JSON.stringify(body);
	if (Buffer.byteLength(json) < REQUEST_COMPRESSION_MIN_BYTES) return json;
	const zlib = process.getBuiltinModule?.("node:zlib") as ZstdModule | undefined;
	if (!zlib || typeof zlib.zstdCompressSync !== "function") return json;
	try {
...
export async function callRemoteCompaction(params: {
	...
	const timeoutMs = params.timeoutMs ?? REMOTE_COMPACTION_TIMEOUT_MS;
	const timeout = setTimeout(() => {
		controller.abort(new NonRetryableCompactionError(
			`OpenAI Codex compaction timed out after ${Math.ceil(timeoutMs / 1000)} seconds.`,
		));
	}, timeoutMs);

	try {
		const headers = new Headers(params.headers);
		const body = encodeRequestBody(params.body, headers);
		let lastError: unknown;
		for (let attempt = 0; attempt <= MAX_REMOTE_RETRIES; attempt++) {
			try {
				const response = await abortable(fetchImpl(params.url, {
					method: "POST",
					headers,
				...
				if (!response.ok) {
					const body = await abortable(response.text().catch(() => ""), controller.signal);
					...
					lastError = error;
					await delay(parseRetryDelay(response) ?? 1000 * 2 ** attempt, controller.signal);
					continue;
				}
				const parsed = await parseSseResponse(response, controller.signal);
				return { compactionItem: parsed.item, usage: usageFromResponse(params.model, parsed.usage) };
			} catch (error) {
				if (controller.signal.aborted || error instanceof NonRetryableCompactionError) throw error;
				lastError = error;
				if (attempt === MAX_REMOTE_RETRIES) throw error;
