...
class BackgroundBashManager {
	...
	private spawnTmux(command: string, cwd: string): TmuxJob {
		...
		try {
			runTmux(["new-session", "-d", "-s", tmuxSession, "-c", cwd, shellPath(), scriptFile]);
		...
		if (existsSync(job.statusFile)) {
			if (tmuxSessionExists(job.tmuxSession)) {
				for (let attempt = 0; attempt < 100; attempt++) {
					const panes = spawnSync("tmux", ["list-panes", "-t", job.tmuxSession, "-F", "#{pane_dead}"], { encoding: "utf8" });
					if (panes.status !== 0 || panes.stdout.trim() === "1") break;
					spawnSync("sleep", ["0.01"]);
				}
				const pane = spawnSync("tmux", ["capture-pane", "-p", "-J", "-t", job.tmuxSession, "-S", `-${TMUX_HISTORY_LINES}`], { encoding: "utf8" });
...
export default function bgBashExtension(pi: ExtensionAPI) {
	...
	pi.registerTool(withStatusCard({
		...
		async execute(_toolCallId, params, signal, onUpdate) {
			...
			const jobExitWait = manager.waitForJobExit(signal);
			const asyncCompletionWait = waitForAsyncCompletion(signal);
			const externalWakeWait = waitForExternalWake(signal);
