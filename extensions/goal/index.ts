...
const toGoal = (data: GoalState | undefined): Goal | undefined => {
	...
	return {
		...
		activeSince: status === "active" ? (state.activeSince ?? timestamp) : null,
		createdAt: state.createdAt ?? timestamp,
		updatedAt: state.updatedAt ?? state.createdAt ?? timestamp,
	};
};
...
export default function (pi: ExtensionAPI) {
	...
	const queueContinuation = (ctx?: { isIdle(): boolean; hasPendingMessages(): boolean }) => {
		if (!goal || goal.status !== "active" || continuationQueued || compactionInProgress) return;
	...
	pi.on("agent_end", async (event, ctx) => {
		const interrupted = activeGoalTurn && !compactionInProgress && runWasInterrupted(event.messages);
	...
	pi.on("session_compact_failed", async (event, ctx) => {
		...
		setGoalStatus(ctx);
		ctx.ui.notify(
			`Goal paused because compaction failed${event.errorMessage ? `: ${event.errorMessage}` : "."}`,
			"warning",
		);
	});
