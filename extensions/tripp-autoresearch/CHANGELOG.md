...
- Replaced the generic optimization guidance with the benchmark-winning jump-climb strategy. Active autoresearch sessions now receive it in every system prompt, including setup and post-compaction turns.
- Distinguished run-now requests from setup-only handoffs. When the user will start the loop manually, the agent now prepares and validates the session without initializing it or creating the experiment log.
- Kept finalization as a separate user-requested job.
