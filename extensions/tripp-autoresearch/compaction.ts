...
function recentRunsSection(state: ReconstructedJsonlState, workDir: string, jsonlPath: string): string {
  ...
  if (runs.length === 0) {
    return "## Recent Runs\n\nNo runs yet. Measure the untouched baseline first.";
  }
...
function nextStepSection(): string {
  return [
    "## Next Step",
    "",
    "Rank the ideas backlog and latest `next:` hints by plausible relative improvement.",
    "Test the highest-impact viable idea next. Do not stop until interrupted.",
  ].join("\n");
}
